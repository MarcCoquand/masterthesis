...
it is possible to couple dependencies with logic. This study will create
applications specifically for the study itself, which means that maintainability
is not of high concern. But since the design patterns might affect the metrics
we will be looking at, we still employ them in this study.  This causes problems
if we later want to change the dependency since that means we also have to
change the logic. To increase maintainability, we want to use a design patterns.
