...
the input, what the output should be of that type. A GADT are good for
implementing \it{domain-specific languages} (DSL).A DSL is a language with a
limited scope for specific applications. For example a parsing library.
...
expressions, for example \lstinline{(Number 5 `Add` Number 3) `Multiply`
(Number 4 `Add` Number 3)}. This expression can translates to $(5+3)*(4+3)$.
Lastly we need to define a way to evaluate the expression.
