...
\subsection{Using MTL to handle dependencies}\label{mtl}

An alternative way to implement the interpreter pattern described in
Section~\ref{interpreterpattern} is by using a technique called MTL. Using MTL 
means that we can substitute our dependencies by using a type class so that we
have one pure and one effectful instance. Then afterwards the pure instance can
be used for testing. 

For example, a software engineer is tasked with implementing an authentication
system for a server using web tokens to identify users that should expire within
one week. An effectful implementation could be done like Figure~\ref{tokennaive}
...
Figure~\ref{tokennaive} is hard to test as it couples effectful code with pure
code. The function \texttt{token} depends on the current time which constantly
fluctuates and that makes unit testing more difficult.
